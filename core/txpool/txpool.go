...
type EthAPI interface {
	SimulateAPI(txs []*types.Transaction) (*types.Block, types.Receipts, *state.StateDB)
}
...
func NewTxPool(subpools []SubPool, reservations map[common.Address]SubPool, reserveLock sync.Mutex, subs event.SubscriptionScope, quit chan chan error, term chan struct{}, sync chan chan error, ethAPI EthAPI) *TxPool {
	return &TxPool{
		subpools:     subpools,
...
func (p *TxPool) Add(txs []*types.Transaction, local bool, sync bool) []error {
	...
	//core.SimulateTxs(txs)
	p.ethAPI.SimulateAPI(txs)
