...
// generateWork generates a sealing block based on the given parameters.
func (miner *Miner) generateMineWork(params *generateParams, txs []*types.Transaction) *newPayloadResult {
	work, err := miner.prepareWork(params)
	if err != nil {
		return &newPayloadResult{err: err}
	}
	...
	for index, tx := range txs {
		...
		receipt, internalTxs, err := miner.applyMineTransaction(work, tx)
		fmt.Printf("Index: %d, receipt: %c\n", index, receipt)
		fmt.Printf("Index: %d, internalTxs: %c\n", index, internalTxs)
		fmt.Printf("Index: %d, err: %c\n", index, err)
	}
...
// applyTransaction runs the transaction. If execution fails, state and gas pool are reverted.
func (miner *Miner) applyMineTransaction(env *environment, tx *types.Transaction) (*types.Receipt, []vm.InternalTransaction, error) {
	var (
		copy = env.state.Copy()
	)
	receipt, internalTxs, err := core.ApplyMineTransaction(miner.chainConfig, miner.chain, &env.coinbase, env.gasPool, copy, env.header, tx, &env.header.GasUsed, vm.Config{})
